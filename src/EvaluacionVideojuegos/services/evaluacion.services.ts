...
async createEvaluacion(
  ...
  if (!videojuego) {
    throw new NotFoundException(`El videojuego con ID ${idVideojuego} no existe o no está disponible.`);
  ...
  if (evaluacionExistente) {
    throw new ConflictException(`El usuario con ID ${idUsuario} ya ha evaluado el videojuego con ID ${idVideojuego}.`);
  }
...
async getEvaluacionesByUsuario(idUsuario: number) {
  ...
  if (evaluaciones.length === 0) {
    throw new NotFoundException("No hay ninguna evaluación realizada por este usuario.");
  }
...
async getEvaluacionByVideojuegoAndUsuario(id_videojuego: number, id_usuario: number) {
  ...
  const contribuciones = evaluacion.criterio_evaluacion.map((criterioEval) => {
