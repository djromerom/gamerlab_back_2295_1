...
async getAllVideojuegos() {
  ...
  if (videojuegos.length === 0) {
    throw new NotFoundException("No se encontraron videojuegos en la base de datos.");
  }
...
async getVideojuegoById(id_videojuego: number, id_usuario: number) {
  ...
  if (!videojuego) {
    throw new NotFoundException(`No se encontró el videojuego con ID ${id_videojuego}.`);
  }
...
async getVideojuegosNoEvaluadosPorUsuario(id_usuario: number) {
  ...
  if (videojuegosNoEvaluados.length === 0) {
    throw new NotFoundException(`Todos los videojuegos han sido evaluados por el usuario con ID ${id_usuario}.`);
  }
